...
class LatentEventFusion(nn.Module):
    ...
    def forward(self, conditions_latent, evs_latents, conditions_latent_ref):
        ...
        # average_feature = (x_conditions_latent + x_evs_latents + x_conditions_latent_ref)/3.0
        ...
        p = self.channel_attention1(x) * x + self.channel_attention1_evs(x_evs_latents) * x_evs_latents
        x = self.residual_block2(p) + x
        p = self.channel_attention2(x) * x + self.channel_attention2_evs(x_evs_latents) * x_evs_latents
        x = self.residual_block3(p) + x
        p = self.channel_attention3(x) * x + self.channel_attention3_evs(x_evs_latents) * x_evs_latents
        x = self.residual_block4(p) + x
        p = self.channel_attention4(x) * x #+ self.channel_attention4_evs(x_evs_latents) * x_evs_latents
        x = self.residual_block5(p) + x
        p = self.channel_attention5(x) * x #+ self.channel_attention5_evs(x_evs_latents) * x_evs_latents
        x = self.residual_block6(p) + x
        p = self.channel_attention6(x) * x #+ self.channel_attention6_evs(x_evs_latents) * x_evs_latents
        x = self.residual_block7(p) + x
        x = x + x_evs_latents
        evs_out = self.output_conv(x)
        ...
        w_cond = torch.zeros(T, device=device)
        w_ref = torch.zeros(T, device=device)
        
        w_evs[0],w_evs[-1] = 0, 0 
        w_ref[0], w_cond[-1] = 1, 1
        ...
        x = w_evs * evs_out + w_cond * conditions_latent + w_ref * conditions_latent_ref
